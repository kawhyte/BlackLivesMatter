...
function Labels(props) {
  const { labels, onClicked, activeButton, type, bgColor } = props
                  ...
                  <span key={i} className="inline-block text-sm mr-2">
                    {item === activeButton ? (
                      ...
                      <button
                        onClick={e => onClicked({ item })}
                        className={
                          "cursor-pointer m-1 text-xs font-semibold inline-block py-1 px-2 border rounded-lg bg-white hover:bg-black hover:text-white text-gray-900 uppercase"
                        }
                      >
                        {item}
                      </button>
